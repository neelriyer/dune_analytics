...
SELECT date_trunc('day', evt_block_time) AS _day,
       sum("positionNotional"/10^18) AS _positionNotionalLiquidated,
...
WHERE ("liquidationPenalty" > 0 or "badDebt" > 0) 
-- AND "evt_block_time" >= now() - interval '30 days'
GROUP BY 1,3, "trader_type"
